...
def load_and_preprocess_images_square(image_path_list, target_size=1024):
    ...
    for image_path in image_path_list:
        ...
        images.append(to_tensor(square_img))
...
def load_and_preprocess_images(image_path_list, mode="crop", target_size=518, patch_size=14):
    """Load images using VGGT preprocessing with configurable target and patch sizes."""
    if len(image_path_list) == 0:
        raise ValueError("At least 1 image is required")
    if mode not in ["crop", "pad"]:
        raise ValueError("Mode must be either 'crop' or 'pad'")
    if target_size <= 0 or patch_size <= 0:
